# Mark larsen 4/12/17 
# union source list ID parsing. - finds gaps in ID's as well as duplicates.
use strict;
use warnings;
my @ID; #multi key
my @baseID; #non - 3 dimensional array - only contains ID's

my $gapFile = "gapInfo2.txt";
my $dupFile = "duplicateInfo2.txt";

#file Headers
open my $headfile, '>>', $gapFile;
print $headfile "Line #| (ID) gapStart| (ID) gapEnd| note| lines:\n";
close $headfile;

open my $headfile1, '>>', $dupFile;print $headfile1 "Dup ID| 1st Line| 1st Sheet| 2nd Line| 2nd Sheet\n";close $headfile1;
...
my $filename = 'unionlist_edited.txt'; #unionSourcelist.txt
